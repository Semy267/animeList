...
export default function AnimeList({ title, linkHref, fetchType }: AnimeListProps) {
  ...
  return (
    <div className="px-4 max-w-[1200px] mx-auto pt-[80px]">
      <div className="flex justify-between mb-5 items-center relative h-fit">
        <h1 className="text-2xl font-bold before:absolute before:w-1 before:rounded-md before:h-full before:bg-white pl-6 before:left-3">{title}</h1>
        <Link href={linkHref} className="text-primary hover:underline hover:text-zinc-300 transition-all duration-100">
          Lihat Semua
        </Link>
      </div>
